...
function getDOM(link, thumbLink, name, description){
	let dom = 
	`<div class='services' onclick="window.location.href ='${link}'">
		<div class='box'>
			<div class='iconbox'>
				<img src='${thumbLink}'>
			</div>
...
async function setWebpage(){
	...
	for(let i = 0; i < data.length; i++){
		contentHTML += getDOM(data[i].link, data[i].thumbLink, data[i].name, data[i].description);
	}
...
function showSearchedProjects(text){
	...
	let searchedData = data.filter(project => project.name.toLowerCase().includes(text) || project.description.toLowerCase().includes(text))
	let elements = '';
	for(let i = 0; i < searchedData.length; i++){
		elements += getDOM(searchedData[i].link, searchedData[i].thumbLink, searchedData[i].name, searchedData[i].description);
	}
